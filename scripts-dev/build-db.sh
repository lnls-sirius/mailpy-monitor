...
TAG=carneirofc/mailpy-db:${COMMIT}-${DATE}
docker build \
    ...
    --label "org.opencontainers.image.description='EPICS Alarm Server Database'" \
    --label "org.opencontainers.image.title='EPICS Alarm Server Database'" \
    --target db \
    --tag ${TAG} \
    .
